                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.promos.update', $promo) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="branch_id" name="branch_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ old('branch_id', $promo->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="ingredient_id" name="ingredient_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                @foreach ($ingredients as $ingredient)
                                    <option value="{{ $ingredient->id }}" {{ old('ingredient_id', $promo->ingredient_id) == $ingredient->id ? 'selected' : '' }}>{{ $ingredient->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="recipe_id" name="recipe_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach ($recipes as $recipe)
                                    <option value="{{ $recipe->id }}" {{ old('recipe_id', $promo->recipe_id) == $recipe->id ? 'selected' : '' }}>{{ $recipe->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="promo_text" name="promo_text" type="text" class="mt-1 block w-full" :value="old('promo_text', $promo->promo_text)" placeholder="e.g. Fiesta Offer, Holy Week Saver" required />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="valid_from" name="valid_from" type="date" class="mt-1 block w-full" :value="old('valid_from', $promo->valid_from->format('Y-m-d'))" required />
                                <x-input-error class="mt-2" :messages="$errors->get('valid_from')" />
                            </div>
                            <div>
                                <x-input-label for="valid_until" :value="__('Valid Until')" />
                                <x-text-input id="valid_until" name="valid_until" type="date" class="mt-1 block w-full" :value="old('valid_until', $promo->valid_until->format('Y-m-d'))" required />
                        ...
                        <div>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_active', $promo->is_active) ? 'checked' : '' }}>
